...
def create_shot_adapter_conditioning_mask(
    ...
    In shot-adapter:
    - Previous shot (xp): Full conditioning (strength 1.0)
    - Stable tokens in prev shot: Medium conditioning (0.5)
    - Transition token: Strong conditioning (0.8)
    - Current shot (x1c): NO conditioning (0.0) - these are training targets
    - Stable tokens in curr shot: Medium conditioning (0.5)
    ...
    Returns:
        Conditioning mask [Total_F] with appropriate strengths
    """
    total_frames = metadata['total_frames']
    mask = torch.zeros(total_frames)

    logger.info(f"🎭 Creating shot-adapter conditioning mask for {total_frames} frames")
    ...
    for i in range(prev_start, prev_end):
        ...
        if local_idx % 2 == 0:  # Frame
            mask[i] = conditioning_strength
        else:  # Stable token
            mask[i] = stable_token_strength

    logger.info(f"🎭   Previous shot [{prev_start}:{prev_end}] → frames: {conditioning_strength}, stables: {stable_token_strength}")

    # Transition token: strong conditioning
    mask[trans_start:trans_end] = transition_token_strength
    logger.info(f"🎭   Transition token [{trans_start}:{trans_end}] → strength {transition_token_strength}")
    ...
    logger.info(f"🎭   Current shot [{curr_start}:{curr_end}] → frames: 0.0 (targets), stables: {stable_token_strength}")
    logger.info(f"🎭   Final mask: {mask.tolist()}")
...
def create_token_aware_conditioning_mask(
    ...
    Returns:
        Conditioning mask [Total_F] with appropriate strengths for each token type
    """
    total_frames = metadata['total_frames']
    mask = torch.ones(total_frames)

    logger.info(f"🎭 Creating token-aware conditioning mask for {total_frames} frames")
    ...
    mask[prev_start:prev_end] = conditioning_strength
    logger.info(f"🎭   Previous shot [{prev_start}:{prev_end}] → strength {conditioning_strength}")
    ...
    for i in range(curr_frames):
        ...
        if local_idx % 2 == 1:  # Stable token
            mask[global_idx] = stable_token_strength
            stable_positions.append(global_idx)
        else:  # Regular frame - these should typically have low/zero conditioning for training
            mask[global_idx] = 0.0  # Current frames are typically not conditioned during training
            frame_positions.append(global_idx)

    logger.info(f"🎭   Current shot stable tokens {stable_positions} → strength {stable_token_strength}")
    logger.info(f"🎭   Current shot frames {frame_positions} → strength 0.0 (training targets)")
    logger.info(f"🎭   Final conditioning mask: {mask.tolist()}")
...
def create_scenario_conditioning_mask(
    ...
    Args:
        ...
        target_shot: If specified, this shot has 0.0 conditioning (training target)
    ...
    logger.info(f"🎭 Creating scenario conditioning mask for {total_frames} frames")
    if target_shot:
    ...
    for shot_name, (start, end) in metadata['shot_ranges'].items():
        if target_shot and shot_name == target_shot:
            # Target shot: set frames to 0.0, stable tokens to medium
            for i in range(start, end):
                shot_local_idx = i - start
                if shot_local_idx % 2 == 0:  # Frame
                    mask[i] = 0.0  # Training target
                else:  # Stable token
                    mask[i] = stable_token_strength
            logger.info(f"🎭   Shot '{shot_name}' [{start}:{end}] → frames: 0.0 (target), stables: {stable_token_strength}")
        else:
            # Conditioning shot: set frames to full, stable tokens to medium
            for i in range(start, end):
                shot_local_idx = i - start
                if shot_local_idx % 2 == 0:  # Frame
                    mask[i] = conditioning_strength
                    ...
                    mask[i] = stable_token_strength
            logger.info(f"🎭   Shot '{shot_name}' [{start}:{end}] → frames: {conditioning_strength}, stables: {stable_token_strength}")

    # Set stable token positions
    for pos in metadata['stable_token_positions']:
        mask[pos] = stable_token_strength

    # Set transition token positions
    for pos in metadata['transition_token_positions']:
        mask[pos] = transition_token_strength
